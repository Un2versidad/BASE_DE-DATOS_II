...
SELECT 
    LOWER(HEX(e.id)) AS id,
    LOWER(HEX(e.aseguradora_id)) AS aseguradora_id,
    a.nombre AS aseguradora_nombre,
    LOWER(HEX(e.juzgado_id)) AS juzgado_id,
    ...
    e.deleted_at
FROM expedientes e
INNER JOIN aseguradoras a ON e.aseguradora_id = a.id
INNER JOIN juzgados j ON e.juzgado_id = j.id
WHERE e.deleted_at IS NULL
  AND a.deleted_at IS NULL
  AND j.deleted_at IS NULL;

CREATE VIEW v_estadisticas_por_estado AS
SELECT 
    estado,
    COUNT(*) AS total
FROM expedientes
